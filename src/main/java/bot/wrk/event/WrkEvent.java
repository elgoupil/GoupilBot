...
public class WrkEvent {
    ...
    public static void eventWrk(Event event, JDA jda, Music musicbot) {
                ...
                if (((MessageReceivedEvent) event).getAuthor() != jda.getSelfUser()) {
                    ...
                    if ("play".equalsIgnoreCase(command[0])) {
                        if (command.length == 2) {
                            ...
                            ((MessageReceivedEvent) event).getChannel().sendMessage("Usage : `play 'url'` or `play 'name of the video on YT'`").queue();
                        }
                    }
                    ...
                    if ("summon".equalsIgnoreCase(command[0])) {
                        musicbot.connectToVoiceChannel(((MessageReceivedEvent) event).getGuild().getAudioManager(), ((MessageReceivedEvent) event).getMember());
